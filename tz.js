...
function calculateLeaderboardPlaces(users, minScores) {
  ...
  const minScoresArr = [minScores.firstPlaceMinScore, minScores.secondPlaceMinScore, minScores.thirdPlaceMinScore]
  let nextPlace = 1 
  return users.map(user => user).sort((a, b) => b.score - a.score).map(user => {
    
    while(minScoresArr.length){
        if(user.score >= minScoresArr[0]){
            minScoresArr.shift()
            return{
                userId: user.userId,
                place: nextPlace++
 ...
 * Пример4
 */
let result4 = calculateLeaderboardPlaces([{ userId: "id1", score: 50 }], {
    firstPlaceMinScore: 100,
    secondPlaceMinScore: 50,
    thirdPlaceMinScore: 10
  });
  console.log("test4", checkResult(result4, [{ userId: "id1", place: 2 }]));
